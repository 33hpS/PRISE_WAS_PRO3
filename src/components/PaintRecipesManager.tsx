...
async function tryLoadFromDb(): Promise<PaintRecipe[]> {
  ...
  return (data || []).map((r: any) => {
    ...
    const parsed = parseNotes<{ ratio?: { base?: number; hardener?: number; thinner?: number } }>(
      notes
    )
    return {
      id: r.id,
...
function computeMixtureCostPerG(params: {
  ...
  const parts = [
    ...
    {
      perG: hardener ? unitPriceToPerGram(hardener.unit, hardener.price) : null,
      share: ratio.hardener,
    },
    {
      perG: thinner ? unitPriceToPerGram(thinner.unit, thinner.price) : null,
      share: ratio.thinner,
    },
...
export default function PaintRecipesManager(): JSX.Element {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const item: PaintRecipe = {
      ...
      notes: JSON.stringify({
        ratio: { base: ratioBase || 0, hardener: ratioHardener || 0, thinner: ratioThinner || 0 },
        base: baseMat
          ? { id: baseMat.id, name: baseMat.name, unit: baseMat.unit, price: baseMat.price }
          : null,
        hardener: hardenerMat
          ? {
              id: hardenerMat.id,
              name: hardenerMat.name,
              unit: hardenerMat.unit,
              price: hardenerMat.price,
            }
          : null,
        thinner: thinnerMat
          ? {
              id: thinnerMat.id,
              name: thinnerMat.name,
              unit: thinnerMat.unit,
              price: thinnerMat.price,
            }
          : null,
      }),
  ...
  /** Формат сом */
  const kgs = (n?: number | null) =>
    n && n > 0 ? `${Math.round(n).toLocaleString('ru-RU')} сом` : '—'
              ...
              <form onSubmit={handleCreate} className='px-6 py-5 space-y-5'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                  ...
                  <div className='space-y-1.5'>
                    ...
                    <Input
                      ...
                      value={costPerG}
                      onChange={e => {
                        setCostPerG(e.target.value)
                        setCostDirty(true)
                      }}
                      placeholder='например, 0.9'
                    />
                    <div className='text-xs text-gray-500 min-h-[32px]'>
                      Если включён авторасчёт — здесь автоматически появится значение из выбранных
                      материалов и соотношения.
                    </div>
                  </div>
                ...
                <div className='flex flex-wrap items-center gap-2 text-sm'>
                  ...
                  <span className='text-gray-500'>
                    Формула:{' '}
                    {pricePerM2 && Number(pricePerM2) > 0 ? 'прямая цена' : 'цена/г × расход'}
                  </span>
                </div>
                    ...
                    return (
                      <TableRow key={r.id} id={`tabpanel-${r.id}`}>
                        <TableCell className='font-medium'>
                          {r.colorName}
                          <div className='text-xs text-gray-500'>A:B:Th = {ratioText}</div>
                        </TableCell>
                        <TableCell
                          title={`Основа: ${r.base || '—'} | Отвердитель: ${r.hardener || '—'} | Разбавитель: ${r.thinner || '—'}`}
                        >
                          {r.coatingType || '—'}
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className='text-right'>{kgs(price)}</TableCell>
